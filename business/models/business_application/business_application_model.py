...
def scaffold_pre_save(sender, instance, created=False, **kwargs):
  ...
  if created:
    if BusinessApplication.objects.filter(business=instance.business, approved=True, created__range=[f'{year}-01-01 00:00:00', f'{year}-12-31 00:00:00']).count() > 0:
      raise IntegrityError("Business Application already approved!")
    elif BusinessApplication.objects.filter(business=instance.business, approved=False, created__range=[f'{year}-01-01 00:00:00', f'{year}-12-31 00:00:00']).count() > 0:
