...
class AdminDashboardBuildingCreateNewView(LoginRequiredMixin, IsAdminViewMixin, View):
    """
    Create view for Buildings.
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(request, "building/new_building.html", context)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if building:
                messages.success(request, 'Building created!', extra_tags='success')
                return HttpResponseRedirect(
                    reverse('admin_dashboard_business_create_by_building', kwargs={'pk': building.pk}))
            else:
                messages.error(request, building_message, extra_tags='danger')
                request.session['building_formdata'] = building_formdata
        else:
